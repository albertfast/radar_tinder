...
async function getFallback(oLat: number, oLng: number, dest: string) {
...
async function searchNearbyPlaces(lat: number, lng: number, radius = 5000, keyword = 'speed_camera|traffic_enforcement') {
  ...
  try {
    ...
    console.error('GoogleMapsService.searchNearbyPlaces error:', error);
    return [];
  }
}
...
// Keep a backward compatible signature just in case it's used directly
async function getSpeedLimitForCoordinate(latitude: number, longitude: number) {
  const places = await searchNearbyPlaces(latitude, longitude, 50, 'road');
  if (places && places.length > 0) {
    return getSpeedLimit(places[0].place_id);
  }
  return null;
}

async function getDistance(oLat: number, oLng: number, dLat: number, dLng: number) {
  try {
...
async function getDirections(oLat: number, oLng: number, dest: string, opts?: { alternatives?: boolean; prefer?: string }): Promise<any> {
  ...
  if (data.status !== 'OK') return data.status === 'ZERO_RESULTS' ? { error: 'ZERO_RESULTS', message: 'No route' } : getFallback(oLat, oLng, dest);
...
async function recalculateRoute(lat: number, lng: number, dest: string, oldRoute?: any) {
  const newRoute = await getDirections(lat, lng, dest, { alternatives: true });
...
async function getGeocodeSuggestions(q: string, opts?: { countryCode?: string; focusLocation?: any }): Promise<AddressSuggestion[]> {
  if (!q.trim()) return [];
